...
- (void)setCornerShadowColor:(UIColor *)color
                  ...
                  blurRadius:(CGFloat)blurRadius
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (index - 1 >= 0 ) {
            shadowView = self.superview.subviews[index - 1];
        }
        if (shadowView && shadowView.tag == 3000) {
        ...
        shadowView.layer.shadowRadius = blurRadius;
        shadowView.layer.masksToBounds = NO;
        shadowView.layer.shadowPath = [UIBezierPath bezierPathWithRoundedRect:self.bounds cornerRadius:self.layer.cornerRadius].CGPath;
    });
}
