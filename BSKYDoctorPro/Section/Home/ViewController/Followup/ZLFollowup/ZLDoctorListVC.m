...
@interface ZLDoctorListVC ()<UITableViewDataSource ,UITableViewDelegate,UITextFieldDelegate>
...
@property (nonatomic ,strong) ZLDoctorListRequest *doctorRequest;
...
- (void)loadDoctorData
{
    if (!self.doctorRequest) {
        self.doctorRequest = [[ZLDoctorListRequest alloc]init];
        self.doctorRequest.pageSize = @"10";
    }
    self.doctorRequest.key = self.textField.text;
    self.doctorRequest.pageIndex = [NSString stringWithFormat:@"%lu",(unsigned long)self.currentPage];
    Bsky_WeakSelf
    [MBProgressHUD showHud];
    [self.doctorRequest startWithCompletionBlockWithSuccess:^(__kindof ZLDoctorListRequest * _Nonnull request) {
        Bsky_StrongSelf
    ...
    } failure:^(__kindof ZLDoctorListRequest * _Nonnull request) {
        Bsky_StrongSelf
        [MBProgressHUD hideHud];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZLDoctorModel *model = self.dataList[indexPath.row];
