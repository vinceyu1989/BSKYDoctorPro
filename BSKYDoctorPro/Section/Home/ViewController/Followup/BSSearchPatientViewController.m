...
@interface BSSearchPatientViewController ()<UITableViewDataSource ,UITableViewDelegate,UITextFieldDelegate>
...
@implementation BSSearchPatientViewController
...
- (void)loadData {
    if (!self.request) {
        if (self.type != FollowupTypeGaoTang) {
            self.request = [BSFollowupSearchRequestNew new];
            if (self.type == FollowupTypeHypertension) {
                [self.request setValue:@"高血压" forKey:@"buildType"];
            }else if (self.type == FollowupTypeDiabetes){
                [self.request setValue:@"糖尿病" forKey:@"buildType"];
            }
            
        }else{
            self.request = [BSFollowupSearchRequest new];
        }
        
    }
    self.currentPage++;
    [self.request setValue:self.textField.text forKey:@"key"];
    [self.request setValue:@(self.currentPage) forKey:@"pageIndex"];
    [self.request setValue:@10 forKey:@"pageSize"];
//    self.request.key = self.textField.text;
//    self.request.pageIndex = @(self.currentPage);
//    self.request.pageSize = @10;
    [MBProgressHUD showHud];
    @weakify(self);
    
    [self.request startWithCompletionBlockWithSuccess:^(BSFollowupSearchRequest* request) {
    ...
    } failure:^(BSFollowupSearchRequest* request) {
        [MBProgressHUD hideHud];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FollowupUserSearchModel *model = self.dataList[indexPath.section];
    ...
    vc.backAllBlock = ^(NSString *sourceRecID) {
        BSHealthTeachVC *health = [[BSHealthTeachVC alloc] init];
        BSEducationModel *info = [[BSEducationModel alloc] init];
        info.personId = followupPlan.gwUserId;
        info.personName = followupPlan.username;
        info.SourceRecID = sourceRecID;
        health.model = info;
...
- (BSFollowup *)configFollowupPlanModelWithFollowupUserSearchModel:(FollowupUserSearchModel*)model {
    ...
    followupPlan.address = model.currentAddress;
    followupPlan.username = model.name;
    followupPlan.userIdCard = model.idCard;
