...
@interface SignRefereeViewController () <UITextFieldDelegate, SingleTableDelegate, FamilyTableDelegate, FamilyPersonViewDelegate> {
    NSString *_temMaster;
}
...
@property (nonatomic, strong) SignFamilyTableView  *familyTableView;
@property (nonatomic, strong) SignFamilyPersonView *familyShowView;
...
@property (nonatomic, strong) BSSignFamilyArchiveRequest *familyInfoRequest;
...
@implementation SignRefereeViewController
...
- (void)initView {
    ...
    self.singleTableView = [[SignSingleTableView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, self.srollView.height) style:UITableViewStylePlain];
    ...
    [self.srollView addSubview:self.singleTableView];
...
- (void)requestToInfo {
    ...
    if (self.segmentControl.selectedSegmentIndex == 0) {
        ...
        self.familyTableView.mj_footer.hidden = NO;
        self.familyInfoRequest.familyCodeOrName = self.textField.text;
        self.familyInfoRequest.pageIndex = @(self.familyIndex);
        [self.familyInfoRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
            [MBProgressHUD hideHud];
            if (weakSelf.familyInfoRequest.familyArchiveData.count != 0) {
                [weakSelf.familyTableView.mj_footer endRefreshing];
                if (weakSelf.familyIndex <= 1) {
                    [weakSelf.familyTableView.tableData removeAllObjects];
                }
                [weakSelf.familyTableView.tableData addObjectsFromArray:weakSelf.familyInfoRequest.familyArchiveData];
                [weakSelf.familyTableView reloadData];
            } else {
                [weakSelf.familyTableView.mj_footer endRefreshingWithNoMoreData];
            }
        } failure:^(__kindof YTKBaseRequest * _Nonnull request) {
            [MBProgressHUD hideHud];
            [weakSelf.familyTableView.mj_footer endRefreshing];
