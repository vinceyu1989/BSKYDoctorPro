...
static CGFloat const kArrowImgWidth= 7;
static CGFloat const kTextLabelX = 5;
static CGFloat const kItemCellHeight = 35;
...
- (void)selectedItemAtIndex:(NSInteger)index {
    ...
    if (index < _dataSource.count) {
        NSString *item = _dataSource[index];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self removeBackgroundView];
    if (_selectedBlock) {
        _selectedBlock(self,_dataSource[indexPath.row]);
    }
}
...
- (UIImageView*)arrowImg {
    if (!_arrowImg) {
        ...
        _arrowImg.image = [UIImage imageNamed:@"icon_down_disease"];
