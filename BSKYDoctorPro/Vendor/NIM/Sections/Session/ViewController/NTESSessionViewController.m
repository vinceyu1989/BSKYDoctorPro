...
- (void)onOpenWebView:(NTESLinkAttachment *)attachment {
    ...
    PersonColligationModel *infoModel = [PersonColligationModel mj_objectWithKeyValues:attachment.dataStr];
    homeVC.infoModel = infoModel;
...
- (void)onTapMediaItemLinkPacket:(NIMMediaItem *)item
{
    // 此处模拟测试数据
    Bsky_WeakSelf;
    NTEResidentManageVC *vc = [[NTEResidentManageVC alloc] init];
    vc.selectBlock = ^(PersonColligationModel * _Nonnull model) {
        Bsky_StrongSelf;
        NTESLinkAttachment *attachment = [[NTESLinkAttachment alloc] init];
        [attachment setTitle:[NSString stringWithFormat:@"%@的个人档案",model.name]];
        if (model.cardId.length) {
            [attachment setDescribe:[model.cardId secretStrFromIdentityCard]];
        }else{
            [attachment setDescribe:@"不详"];
        }
        [attachment setDataStr:[model mj_JSONString]];
...
- (BOOL)onTapAvatar:(NIMMessage *)message{
    ...
    {
        vc = [[IMDetailController alloc] initWithUser:userId];
    }
...
#warning IM修改
    ...
    // icon_session_info_normal
    [enterTeamCard setImage:[UIImage imageNamed:@"btn_session_setting"] forState:UIControlStateNormal];
    [enterTeamCard setImage:[UIImage imageNamed:@"btn_session_setting"] forState:UIControlStateHighlighted];
    ...
    // icon_session_info_normal
    [infoBtn setImage:[UIImage imageNamed:@"btn_session_setting"] forState:UIControlStateNormal];
    [infoBtn setImage:[UIImage imageNamed:@"btn_session_setting"] forState:UIControlStateHighlighted];
    [infoBtn sizeToFit];
