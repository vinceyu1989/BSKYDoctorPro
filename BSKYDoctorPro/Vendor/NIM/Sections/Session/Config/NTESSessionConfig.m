...
//    && [_session.sessionId isEqualToString:[[NIMSDK sharedSDK].loginManager currentAccount]];
    // 添加患者资料测试按钮，此处的 onTapMediaItemLinkPacket 在
#warning IM修改
    NIMMediaItem *linkPacket  = [NIMMediaItem item:@"onTapMediaItemLinkPacket:"
                                       normalImage:[UIImage imageNamed:@"bk_media_archives_normal"]
                                     selectedImage:[UIImage imageNamed:@"bk_media_archives_pressed"]
                                             title:@"患者资料"];
    //...
    
    NSArray *items = @[];
    NIMUser *user = [[NIMSDK sharedSDK].userManager userInfo:[BSClientManager sharedInstance].lastUsername];
    IMEXModel *exModel = [IMEXModel mj_objectWithKeyValues:user.userInfo.ext];

    if (exModel.professionType.integerValue == 1) {
        items = @[defaultMediaItems[0],defaultMediaItems[1],linkPacket];
    }else{
        items = @[defaultMediaItems[0],defaultMediaItems[1]];
    }
